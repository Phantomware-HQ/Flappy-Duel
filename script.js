...
function getCoins() {
  return parseInt(localStorage.getItem('fd_coins') || '0');
}
function addCoins(amount) {
  localStorage.setItem('fd_coins', getCoins() + amount);
}
function spendCoins(amount) {
  localStorage.setItem('fd_coins', getCoins() - amount);
}
function getOwnedSkins() {
  try { return JSON.parse(localStorage.getItem('fd_owned') || '["default"]'); }
...
function ownSkin(skinId) {
  ...
  if (!skins.includes(skinId)) {
    skins.push(skinId);
    localStorage.setItem('fd_owned', JSON.stringify(skins));
  }
}
function getActiveSkin() {
  return localStorage.getItem('fd_activeSkin') || 'default';
}
function setActiveSkin(skinId) {
  localStorage.setItem('fd_activeSkin', skinId);
}

function getSkinName(skinId) {
  const skin = SKINS[skinId] || SKINS['default'];
  if (currentLang === 'tr') return skin.nameTr || skin.name;
  if (currentLang === 'de') return skin.nameDe || skin.name;
  return skin.name;
}
function getSkinDesc(skinId) {
  const skin = SKINS[skinId] || SKINS['default'];
  if (currentLang === 'tr') return skin.descTr || skin.desc;
  if (currentLang === 'de') return skin.descDe || skin.desc;
  return skin.desc;
}
...
function playFlap() {
    ...
    try {
        ...
        o.frequency.setValueAtTime(520, ac.currentTime); o.frequency.exponentialRampToValueAtTime(280, ac.currentTime + 0.12);
        g.gain.setValueAtTime(0.18, ac.currentTime); g.gain.exponentialRampToValueAtTime(0.001, ac.currentTime + 0.13);
...
function playScore() {
    ...
    try {
        ...
        o.frequency.setValueAtTime(880, ac.currentTime); o.frequency.setValueAtTime(1100, ac.currentTime + 0.06);
        g.gain.setValueAtTime(0.22, ac.currentTime); g.gain.exponentialRampToValueAtTime(0.001, ac.currentTime + 0.2);
...
function playDeath() {
    if (!soundEnabled) return;
    try {
        const ac = getAC(); const bufSize = ac.sampleRate * 0.5; const buf = ac.createBuffer(1, bufSize, ac.sampleRate); const d = buf.getChannelData(0);
        for (let i = 0; i < bufSize; i++) d[i] = Math.random() * 2 - 1;
        const ns = ac.createBufferSource(); ns.buffer = buf; const ng = ac.createGain();
        ng.gain.setValueAtTime(0.4, ac.currentTime); ng.gain.exponentialRampToValueAtTime(0.001, ac.currentTime + 0.5);
        ns.connect(ng); ng.connect(ac.destination); ns.start(); ns.stop(ac.currentTime + 0.5);
        const o = ac.createOscillator(), g = ac.createGain(); o.connect(g); g.connect(ac.destination); o.type = 'sawtooth';
        o.frequency.setValueAtTime(200, ac.currentTime); o.frequency.exponentialRampToValueAtTime(40, ac.currentTime + 0.5);
...
function playBeep(loud) {
    ...
    try {
        ...
        g.gain.setValueAtTime(0.25, ac.currentTime); g.gain.exponentialRampToValueAtTime(0.001, ac.currentTime + (loud ? 0.4 : 0.18));
...
function spawnParticles(x, y, color, count) {
    for (let i = 0; i < count; i++) {
        ...
        particles.push({ x, y, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed - 1.5, r: 2 + Math.random() * 4, color, alpha: 1, decay: 0.018 + Math.random() * 0.02 });
    }
}
function drawParticles() {
    for (let i = particles.length - 1; i >= 0; i--) {
        const p = particles[i]; p.x += p.vx; p.y += p.vy; p.vy += 0.15; p.vx *= 0.97; p.alpha -= p.decay;
        if (p.alpha <= 0) { particles.splice(i, 1); continue; }
        ctx.save(); ctx.globalAlpha = p.alpha; ctx.fillStyle = p.color; ctx.shadowColor = p.color; ctx.shadowBlur = 10;
...
function drawBird(b, anim, isOpp) {
    ...
    if (skin.rainbow && !isOpp) {
        const hue = (Date.now() / 10) % 360;
        c1 = `hsl(${hue}, 100%, 50%)`; c2 = `hsl(${(hue + 30) % 360}, 100%, 30%)`;
    }
    ...
    gr.addColorStop(0, c1); gr.addColorStop(1, c2); ctx.fillStyle = gr;
...
function mainLoop() {
    ...
    if (gameRunning) {
        ...
        for (let i = pipes.length - 1; i >= 0; i--) {
            ...
            if (!p.passed && p.x + PIPE_W < myBird.x) {
                p.passed = true; myScore++; myPipes++;
                myScoreEl.textContent = myScore; triggerBump(myScoreEl); playScore();
                socket.emit('scoreUpdate', roomCode);
            }
            if (myBird.x + myBird.r > p.x && myBird.x - myBird.r < p.x + PIPE_W) {
                if (myBird.y - myBird.r < p.top || myBird.y + myBird.r > p.top + PIPE_GAP) { killMyBird(); return; }
            }
    ...
    if (dying) {
        ...
        if (dyingTimer > 50 || (particles.length === 0 && flashAlpha <= 0)) {
            dying = false; myDiedTime = Date.now(); socket.emit('gameOver', roomCode); return;
        }
    }
...
function killMyBird() {
    ...
    gameRunning = false; dying = true; dyingTimer = 0; myDiedTime = Date.now(); cancelAnimationFrame(raf);
    playDeath(); flashColor = '#ffffff'; flashAlpha = 0.65;
    spawnParticles(myBird.x, myBird.y, '#ff4444', 20); spawnParticles(myBird.x, myBird.y, '#ffaa00', 12); spawnParticles(myBird.x, myBird.y, '#ffffff', 8);
    raf = requestAnimationFrame(mainLoop);
}
...
function startCountdown(seconds, ps) {
    ...
    gameRunning = false; pipeSet = ps; showOnly('game'); initCanvas(); pipes = buildPipes(pipeSet);
    myScore = 0; oppScore = 0; myFlaps = 0; oppFlaps = 0; myPipes = 0; oppPipes = 0; myAliveMs = 0; oppAliveMs = 0; myDiedTime = 0;
    myScoreEl.textContent = '0'; oppScoreEl.textContent = '0';
    countdownEl.classList.remove('hidden'); let c = seconds; countdownEl.textContent = c; playBeep(false);
    const iv = setInterval(() => { c--; if (c > 0) { countdownEl.textContent = c; playBeep(false); } else { countdownEl.textContent = 'GO!'; playBeep(true); } }, 1000);
    setTimeout(() => {
        clearInterval(iv); countdownEl.classList.add('hidden'); gameStartTime = Date.now();
        if (myBird) myBird.vel = 0; if (oppBird) oppBird.vel = 0;
        gameRunning = true; raf = requestAnimationFrame(mainLoop);
    }, seconds * 1000);
}
function showStats(endData) {
    const now = Date.now(); myAliveMs = myDiedTime ? myDiedTime - gameStartTime : now - gameStartTime;
    statMyPipes.textContent = myPipes; statOppPipes.textContent = oppPipes;
    statMyFlaps.textContent = myFlaps; statOppFlaps.textContent = endData.oppFlaps ?? oppFlaps;
...
aboutBtn.onclick = openModal; modalClose.onclick = closeModal;
aboutModal.onclick = (e) => { if (e.target === aboutModal) closeModal(); };
...
socket.on('opponentDied', () => {
    ...
    spawnParticles(oppBird.x, oppBird.y, '#cc2222', 18); spawnParticles(oppBird.x, oppBird.y, '#ff8800', 8);
...
socket.on('pong', () => {
    const ping = Date.now() - pingStart; pingValueEl.textContent = `${ping} ms`;
    if (ping < 80) { pingIconEl.textContent = '📶'; pingValueEl.style.color = '#0f0'; }
    else if (ping < 150) { pingIconEl.textContent = '📶'; pingValueEl.style.color = '#ff0'; }
    else { pingIconEl.textContent = '📡'; pingValueEl.style.color = '#f00'; }
});
...
langOptions.forEach(opt => {
    opt.onclick = () => {
        const newLang = opt.dataset.lang; if (newLang === currentLang) { langMenu.classList.add('hidden'); return; }
        currentLang = newLang; localStorage.setItem('lang', currentLang);
        langOptions.forEach(o => o.classList.remove('selected')); opt.classList.add('selected');
        updateUILanguage(); langMenu.classList.add('hidden');
    };
});
...
if (coinMsg) {
    ...
    if (statBoxes.length >= 3) { statBoxes[0].querySelector('.stat-label').textContent = '🏁 ' + t('pipes'); statBoxes[1].querySelector('.stat-label').textContent = '🪶 ' + t('flaps'); statBoxes[2].querySelector('.stat-label').textContent = '⏱️ ' + t('time'); }
    const modalTitle = document.querySelector('.modal-title'); if (modalTitle) modalTitle.textContent = t('aboutTitle');
...
function renderMarket() {
    ...
    if (!coinAmount || !skinList) return;
    coinAmount.textContent = getCoins();
    ...
    Object.values(SKINS).forEach(skin => {
        ...
        div.style.cssText = 'display:flex;align-items:center;gap:12px;padding:12px;border:1px solid var(--border);border-radius:10px;background:rgba(255,255,255,0.02);';
        const preview = document.createElement('div');
        preview.style.cssText = `width:40px;height:40px;border-radius:50%;background:${skin.color1};border:2px solid ${skin.color2};flex-shrink:0;`;
        if (skin.rainbow) preview.style.background = 'linear-gradient(90deg,red,orange,yellow,green,blue,indigo,violet)';
        div.appendChild(preview);
        const info = document.createElement('div'); info.style.flex = '1';
        const name = document.createElement('div');
        name.textContent = getSkinName(skin.id); name.style.fontWeight = '700'; name.style.fontSize = '0.95rem';
        info.appendChild(name);
        const desc = document.createElement('div');
        desc.textContent = getSkinDesc(skin.id); desc.style.fontSize = '0.75rem'; desc.style.color = 'var(--muted)';
